...
export async function registerRoutes(
  ...
  app.get("/api/profiles", async (req, res) => {
    const groupId = await resolveGroupId(
      req.query.groupCode as string | undefined,
    );
  ...
  app.post("/api/suggest-words", async (req, res) => {
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              'You generate word lists for a social deduction party game. Return a JSON object with a "words" key containing an array of 50 distinct, common, well-known items that fit the category. Words must be easy to guess and describe in conversation. No duplicates.',
          },
          {
            role: "user",
      ...
      const words: string[] =
        parsed.words || parsed.items || (Object.values(parsed)[0] as string[]);
  ...
  app.post(api.rooms.create.path, async (req, res) => {
    try {
      ...
      if (!Array.isArray(profileIds) || profileIds.length < 3) {
        return res
          .status(400)
          .json({ message: "Select at least 3 players to start" });
      }
      ...
      const room = await storage.createRoom({
        ...
        imposterCount:
          typeof imposterCount === "number" && imposterCount >= 1
            ? imposterCount
            : 1,
  ...
  app.get(api.rooms.get.path, async (req, res) => {
    ...
    const cluesWithPlayers = clues.map((clue) => ({
      ...clue,
      player: players.find((p) => p.id === clue.playerId)!,
    }));
  ...
  app.post(api.rooms.start.path, async (req, res) => {
    ...
    const body = req.body || {};
    const bodySelectedIds: number[] | undefined = Array.isArray(
    ...
    const imposterCount =
      typeof body.imposterCount === "number" && body.imposterCount >= 1
        ? body.imposterCount
    ...
    let selectedCatIds: number[] =
      bodySelectedIds || (room.selectedCategoryIds as number[]) || [];
    ...
    if (!selectedCategory || !selectedWord) {
      return res
        .status(400)
        .json({ message: "No categories available. Check your settings." });
    }
  ...
  app.post("/api/rooms/:code/resolve", async (req, res) => {
    ...
    for (const entry of imposterResults) {
      ...
      if (result === "win") {
        await storage.updateProfile(profileId, {
          imposterWins: profile.imposterWins + 1,
        });
      } else {
        await storage.updateProfile(profileId, {
          imposterLosses: profile.imposterLosses + 1,
        });
      }
    }
    ...
    for (const player of nonImposterPlayers) {
      ...
      if (anyImposterWon) {
        await storage.updateProfile(profile.id, {
          nonImposterLosses: profile.nonImposterLosses + 1,
        });
      } else {
        await storage.updateProfile(profile.id, {
          nonImposterWins: profile.nonImposterWins + 1,
        });
      }
    }
    ...
    if (Array.isArray(badWordProfileIds)) {
      for (const profileId of badWordProfileIds) {
        ...
        if (!profile) continue;
        await storage.updateProfile(profileId, {
          badWordTally: profile.badWordTally + 1,
        });
      }
    }
