import { z } from 'zod';
import { rooms, players, clues, categories, profiles } from './schema';
...
export const api = {
  profiles: {
    ...
    create: {
      ...
      path: '/api/profiles' as const,
      responses: { 201: z.custom<typeof profiles.$inferSelect>() },
    },
    update: {
  ...
  categories: {
    ...
    create: {
      method: 'POST' as const,
      path: '/api/categories' as const,
      input: z.object({ name: z.string().min(1), words: z.array(z.string().min(1)) }),
      responses: {
  ...
  rooms: {
    ...
    get: {
      ...
      responses: {
        200: z.object({
          ...
          players: z.array(z.custom<typeof players.$inferSelect>()),
          clues: z.array(
            z.custom<typeof clues.$inferSelect>().and(z.object({ player: z.custom<typeof players.$inferSelect>() }))
          ),
        }),
    ...
    clue: {
      ...
      input: z.object({ playerId: z.number(), word: z.string().min(1) }),
      responses: {
    ...
    vote: {
      ...
      input: z.object({ voterId: z.number(), votedForId: z.number() }),
      responses: {
        200: z.custom<typeof players.$inferSelect>(),
        400: errorSchemas.validation,
        404: errorSchemas.notFound,
